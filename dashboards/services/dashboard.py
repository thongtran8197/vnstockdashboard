...
class DashboardService:
    ...
    def get_builder_templates(cls, user_id: int) -> list:
        templates = []
        dashboard_templates = StockDashboardBuilderTemplate.objects.filter(added_by=user_id).prefetch_related("templates")
        for dashboard_template in dashboard_templates:
            ...
            template.update(data=dict(datasets=datasets, labels=[1,2,4,5,6,7,8,9]))
